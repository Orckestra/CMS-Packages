<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template mode="BlogList" match="*">
			<span>
				...
				<xsl:choose>
					<xsl:when test="$isBlogList='false'">
						<div class="BlogContent">
							<xsl:copy-of select="mp:ParseXhtmlBodyFragment(@Content)" />
						</div>
						<xsl:if test="@DisplayComments = 'true'">
							<xsl:variable name="blogComments">
								<f:function xmlns:f="http://www.composite.net/ns/function/1.0" name="Composite.Community.Blog.Comments">
					...
					<xsl:otherwise>
						...
						<xsl:if test ="count($Tags)>0">
							<div class="BlogTags">
								SUBJECTS: <xsl:apply-templates mode="TagsList" select="$Tags" />
							</div>
						</xsl:if>
	...
	<xsl:template name="CommentsCount">
		...
		<a href="~/Renderers/Page.aspx{be:GetBlogUrl(@Date, @Title)}?pageId={$pageId}">
			COMMENTS (<xsl:value-of select="$Count" />)
		</a>
	</xsl:template>
	
	<xsl:template name="AddThis">
		<a class="AddThis" href="http://www.addthis.com/bookmark.php?v=250&amp;"><img src="http://s7.addthis.com/static/btn/v2/lg-share-en.gif" width="125" height="16" alt="Bookmark and Share" style="border:0"/></a><script type="text/javascript" src="http://s7.addthis.com/js/250/addthis_widget.js"></script>
	</xsl:template>
