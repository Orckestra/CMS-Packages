			...
			<body>
				<xsl:choose>
					<xsl:when test="$mode='Combination'">
						<script type="text/javascript" src="http://{$forumShortname}.disqus.com/combination_widget.js?num_items={$items}&amp;hide_mods={$moderatorsRanking}&amp;color={$colorTheme}&amp;default_tab={$defaultTabView}&amp;excerpt_length={$excerptLength}"></script>
						<a href="http://disqus.com/">Powered by Disqus</a>
					</xsl:when>
					<xsl:when test="$mode='Popular'">
						<div id="popularthreads" class="dsq-widget">
							<h2 class="dsq-widget-title">Popular Threads</h2>
							<script type="text/javascript" src="http://{$forumShortname}.disqus.com/popular_threads_widget.js?num_items={$items}"></script>
						</div>
						<a href="http://disqus.com/">Powered by Disqus</a>
					</xsl:when>
					<xsl:when test="$mode='Top'">
						<div id="topcommenters" class="dsq-widget">
							...
							<script type="text/javascript" src="http://{$forumShortname}.disqus.com/top_commenters_widget.js?num_items={$items}&amp;hide_mods={$moderatorsRanking}&amp;hide_avatars={$showAvatars}&amp;avatar_size={$avatarSize}"></script>
						</div>
						<a href="http://disqus.com/">Powered by Disqus</a>
					</xsl:when>
					<xsl:otherwise>
						<div id="recentcomments" class="dsq-widget">
							...
							<script type="text/javascript" src="http://{$forumShortname}.disqus.com/recent_comments_widget.js?num_items={$items}&amp;hide_avatars={$showAvatars}&amp;avatar_size={$avatarSize}&amp;excerpt_length={$excerptLength}"></script>
						</div>
						<a href="http://disqus.com/">Powered by Disqus</a>
					</xsl:otherwise>
				</xsl:choose>
