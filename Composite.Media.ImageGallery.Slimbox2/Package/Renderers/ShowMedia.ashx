...
public class ShowMedia : IHttpHandler, IReadOnlySessionState
{
    ...
    public class ResizingOptions
    {
        ...
        public bool IsEmpty
        {
            get
            {
                return Height == null && Width == null && MaxHeight == null && MaxWidth == null && SquareSize == null;
            }
        }
        ...
        public static ResizingOptions FromQueryString(NameValueCollection queryString)
        {
            ...
            if (!string.IsNullOrEmpty(str))
            {
                result.SquareSize = int.Parse(str);
            }
    ...
    private string GetResizedImage(HttpContext context, IMediaFile file, ResizingOptions resizingOptions, ImageType imgType)
    {
        ...
        try
        {
            ...
            if (!C1File.Exists(imageFullPath) || C1File.GetLastWriteTime(imageFullPath) != file.LastWriteTime)
            {
                ...
                ResizeImage(bitmap, imageFullPath, newWidth, newHeight, resizingOptions.SquareSize, imgType);
    ...
    private static void ResizeImage(Bitmap image, string outputFilePath, int newWidth, int newHeight, int? squareSize, ImageType imgType)
    {
        Verify.ArgumentNotNull(image, "image");
        int width = newWidth, height = newHeight, overwidth = 0, overheight = 0;
        if (squareSize != null)
        {
            newHeight = newWidth = (int)squareSize;
            if (image.Width > image.Height)
            {
                height = newHeight;
                width = image.Width * newWidth / image.Height;
                overwidth = (newWidth - width) / 2;
            }
            else
            {
                height = image.Height * newHeight / image.Width;
                width = newWidth;
                overheight = (newHeight - height) / 2;
            }
        }
        using (Bitmap resizedImage = new Bitmap(image, newWidth, newHeight))
        {
            ...
            newGraphic.DrawImage(image, overwidth, overheight, width, height);
    ...
    private static bool CalculateSize(int width, int height, ResizingOptions resizingOptions, out int newWidth, out int newHeight)
    {
        ...
        if (resizingOptions.Height != null && resizingOptions.Width != null || resizingOptions.SquareSize != null)
        {
           
            newHeight = resizingOptions.Height!=null ? (int)resizingOptions.Height : (int)resizingOptions.SquareSize;
            newWidth = resizingOptions.Width!=null ? (int)resizingOptions.Width: (int)resizingOptions.SquareSize;
