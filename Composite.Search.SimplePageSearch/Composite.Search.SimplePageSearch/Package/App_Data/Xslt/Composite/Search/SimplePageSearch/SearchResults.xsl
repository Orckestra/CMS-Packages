...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:in="http://www.composite.net/ns/transformation/input/1.0" xmlns="http://www.w3.org/1999/xhtml" exclude-result-prefixes="xsl in">
	...
	<xsl:param name="showSearchForm" select="/in:inputs/in:param[@name='ShowSearchForm']" />
	<xsl:template match="/">
		<html>
			<head>
				<xsl:if test="$searchQuery != ''">
					<title>
						Results for <xsl:value-of select="concat('&quot;',$searchQuery,'&quot;')" />
					</title>
					<link rel="stylesheet" type="text/css" href="~/Frontend/Composite/Search/SimplePageSearch/Styles.css" />
				</xsl:if>
			</head>
			<body>
				<xsl:if test="$showSearchForm = 'true'">
