...
function rate(score, ratyId) {

    var domRatyId = '#id' + ratyId;
    var jsonData = { "score": score, "ratyId": ratyId};
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var count;
            var total;
            var o = JSON.parse(data.d, function (key, value) {
                if (typeof (value) == "object")
                    return;
                switch (key) {
                    case 'TotalValue': total = value; break;
                    case 'Count': $(domRatyId).next().find(".count").html(value); count = value; break;
                    default: break;
                }
            });
            setAvr(ratyId, total, count);
            $.fn.raty.readOnly(true, domRatyId);
        }
    });

}

function setAvr(ratyId, total, count) {
    var avr = (Number(total) / Number(count)).toFixed(2);
    var domRatyId = '#id' + ratyId;
    $.fn.raty.start(Number(avr), domRatyId);
    $(domRatyId).next().find(".avr").html(avr);
}
