<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
	...
	exclude-result-prefixes="w a o v WX aml w10 wp r"
	>
...
<xsl:variable name="tblStyleSuffix"></xsl:variable>
<xsl:variable name="rowStyleSuffix"></xsl:variable>
<xsl:variable name="cellStyleSuffix"></xsl:variable>
<xsl:variable name="paraStyleSuffix"></xsl:variable>
<xsl:variable name="charStyleSuffix"></xsl:variable>
...
<xsl:template name="GetBorderPr">
        ...
        <xsl:value-of select="@w:sz * 2.5"/><xsl:value-of select="$sep2"/>
			</xsl:when>
			<xsl:otherwise>0<xsl:value-of select="$sep2"/></xsl:otherwise>
		</xsl:choose>
...
<xsl:template name="ApplyShd">
    ...
    <xsl:variable name="backgroundColor">
	    <xsl:choose>
		    ...
		    <xsl:otherwise>
			    <xsl:call-template name="ConvHexColor"><xsl:with-param name="value" select="@WX:bgcolor"/><xsl:with-param name="autoColor" select="$autoColor_bg"/></xsl:call-template>
		    </xsl:otherwise>
	    </xsl:choose>
	</xsl:variable>
...
<xsl:template name="PrsUpdateRPrCore">
	...
	<xsl:for-each select="w:rPr[1]">
		...
		<xsl:variable name="fICs">
			<xsl:for-each select="w:iCs[1]"><xsl:call-template name="EvalBooleanType"/></xsl:for-each>
		</xsl:variable>
		<xsl:choose>
...
<xsl:template name="PrsGetListPrFromListDef">
	...
	<xsl:choose>
		...
		<xsl:when test="$type = $prrListSuff">
			...
			<xsl:choose>
				<xsl:when test="$suff = $prListSuff_space or $suff = $prListSuff_nothing"><xsl:value-of select="$suff"/></xsl:when>

				<xsl:otherwise><xsl:value-of select="$prListSuff_space"/></xsl:otherwise>
			</xsl:choose>
		</xsl:when>

		<xsl:when test="$type = $prrListInd">
			<xsl:for-each select="w:pPr[1]/w:ind[1]">
				<xsl:value-of select="@w:left"/><xsl:value-of select="$sep2"/>
				<xsl:value-of select="@w:left-chars"/><xsl:value-of select="$sep2"/>
				<xsl:value-of select="@w:hanging"/><xsl:value-of select="$sep2"/>
...
<xsl:template name="wrapFrame">
	...
	<xsl:choose>
		...
		<xsl:otherwise>
			...
			<xsl:if test="$wrap = '' or $wrap = 'none' or $wrap = 'not-beside'"><br class="clearAll"/></xsl:if>
		</xsl:otherwise>
	</xsl:choose>
</xsl:template>
        ...
        <xsl:otherwise>
          <xsl:value-of select="w:numPr[1]/w:ilvl/@numString"/>
			...
			<xsl:if test="w:numPr[1]/WX:t/@WX:wTabAfter">
				<span>
					<xsl:attribute name="style">
						<xsl:text>padding-left:</xsl:text><xsl:value-of select="(w:numPr[1]/WX:t/@WX:wTabAfter div 20)" /><xsl:text>pt;</xsl:text>
					</xsl:attribute>						
				</span>
			</xsl:if>
		</xsl:when>	
...
<!--	<xsl:choose>
		...
		<xsl:when test="$b.line-through = $on and $b.underline = $on">text-decoration:none underline;</xsl:when>
...
<xsl:template match="w:i" mode="rpr">
	...
	<xsl:template match="w:u" mode="rpr">
		<xsl:choose>
			<xsl:when test="@w:val = 'off'">font-style:normal;</xsl:when>
			<xsl:otherwise>text-decoration:underline;</xsl:otherwise>
		</xsl:choose>
	</xsl:template>

	<xsl:template match="w:strike" mode="rpr">
		<xsl:choose>
			<xsl:when test="@w:val = 'off'">font-style:normal;</xsl:when>
  ...
  <xsl:variable name="pr.listSuff">
	...
	<xsl:variable name="styleMod">
		...
		<xsl:variable name="isBullets">
			<xsl:for-each select="w:numPr[1]"><xsl:call-template name="IsListBullet" /></xsl:for-each>
		</xsl:variable>
			...
			<xsl:when test="translate($instruction, $lowercase, $uppercase)='HYPERLINK'">
				<a>
					<xsl:variable name="href">
						<xsl:choose>
							<xsl:when test="contains($nInstrText,'\l')">#<xsl:value-of select="translate(substring-before(substring-after($nInstrText, '\l '),' '),'&quot;', '')"/></xsl:when>
							<xsl:otherwise>
...
<xsl:template name="DisplayHlink">
	...
	<xsl:if test="not($href='')"><xsl:attribute name="href"><xsl:value-of select="$href"/></xsl:attribute></xsl:if>
	<xsl:for-each select="@w:tgtFrame"><xsl:attribute name="target"><xsl:value-of select="."/></xsl:attribute></xsl:for-each>
	<xsl:for-each select="@w:tooltip"><xsl:attribute name="title"><xsl:value-of select="."/></xsl:attribute></xsl:for-each>
  ...
  <xsl:if test="$href=''">
	...
	<xsl:call-template name="DisplayPContent"><xsl:with-param name="b.bidi" select="$b.bidi"/><xsl:with-param name="prsR" select="$prsR"/></xsl:call-template>
	</a>
</xsl:template>
...
<!--	<xsl:choose>
	...
	<xsl:if test="not($nInd='' and $pr.listInd='')">
      ...
      <xsl:when test="not($nInd.left = '')"><xsl:value-of select="$marginSide.before"/><xsl:value-of select="$nInd.left div 20"/>pt;</xsl:when>
			<xsl:when test="not($nInd.leftChars = '' and $nInd.hangingChars='')">
				<xsl:value-of select="$marginSide.before"/>
				<xsl:variable name="leftchars"><xsl:choose><xsl:when test="$nInd.leftChars=''">0</xsl:when><xsl:otherwise><xsl:value-of select="$nInd.leftChars div 100"/></xsl:otherwise></xsl:choose></xsl:variable>
				<xsl:variable name="hangingchars"><xsl:choose><xsl:when test="$nInd.hangingChars=''">0</xsl:when><xsl:otherwise><xsl:value-of select="$nInd.hangingChars div 100"/></xsl:otherwise></xsl:choose></xsl:variable>
				<xsl:value-of select="$leftchars + $hangingchars"/>
		...
		<xsl:choose>
			<xsl:when test="not($nInd.right = '')"><xsl:value-of select="$marginSide.after"/><xsl:value-of select="$nInd.right div 20"/>pt;</xsl:when>
			<xsl:when test="not($nInd.rightChars = '')"><xsl:value-of select="$marginSide.after"/><xsl:value-of select="$nInd.rightChars div 100"/>em;</xsl:when>
		</xsl:choose>

		<xsl:choose>
			<xsl:when test="not($nInd.hanging='')">text-indent:<xsl:value-of select="$nInd.hanging div -20"/>pt;</xsl:when>
			<xsl:when test="not($nInd.hangingChars='')">text-indent:<xsl:value-of select="$nInd.hangingChars div -100"/>em;</xsl:when>
			<xsl:when test="not($nInd.firstLine='')">text-indent:<xsl:value-of select="$nInd.firstLine div 20"/>pt;</xsl:when>
			<xsl:when test="not($nInd.firstLineChars='')">text-indent:<xsl:value-of select="$nInd.firstLineChars div 100"/>em;</xsl:when>
...
<xsl:template match="w:spacing[@w:lineRule or @w:line]" mode="ppr">
	<xsl:choose>
		<xsl:when test="not(@w:lineRule) or @w:lineRule = 'exact'">line-height:<xsl:value-of select="@w:line div 20"/>pt;</xsl:when>
	</xsl:choose>
</xsl:template>
...
<xsl:template match="w:p">
	...
	<xsl:if test="not(w:pPr/w:pStyle/@w:val='z-TopofForm') and not(w:pPr/w:pStyle/@w:val='z-BottomofForm')">
		...
		<xsl:variable name="tagName">
			<xsl:choose>
<!--				<xsl:when test="$pStyleId='Heading1'">H1</xsl:when>
				<xsl:when test="$pStyleId='Heading2'">H2</xsl:when>
				<xsl:when test="$pStyleId='Heading3'">H3</xsl:when>
				<xsl:when test="$pStyleId='Heading4'">H4</xsl:when>
				<xsl:when test="$pStyleId='Heading5'">H5</xsl:when>
				<xsl:when test="$pStyleId='Heading6'">H6</xsl:when>	-->
				<xsl:when test="$pStyleId=''">p</xsl:when>
...
<xsl:template match="w:textDirection" mode="tcpr"><xsl:call-template name="ApplyTextDirection"/></xsl:template>
...
<xsl:template match="w:tc">
	...
	<xsl:if test="not($vmerge and not($vmerge/@w:val))">
		...
		<xsl:attribute name="class">
			<xsl:value-of select="$sTblStyleName/@w:styleId"/><xsl:value-of select="$cellStyleSuffix"/>
		</xsl:attribute>
...
<xsl:template match="w:tr">
	...
	<xsl:attribute name="class">
		<xsl:value-of select="$sTblStyleName/@w:styleId"/><xsl:value-of select="$rowStyleSuffix"/>
	</xsl:attribute>
...
<xsl:template name="ApplyTblPr.class">
	<xsl:for-each select="w:tblPr[1]">
		...
		<xsl:for-each select="w:tblW[1]">
			<xsl:if test="@w:type != 'auto'">width:<xsl:call-template name="EvalTableWidth"/>;</xsl:if>
		</xsl:for-each>
	</xsl:for-each>
</xsl:template>
...
<xsl:template name="tblCore">
	...
	<xsl:attribute name="class"><xsl:value-of select="$tStyleId"/><xsl:value-of select="$tblStyleSuffix"/></xsl:attribute>
...
<xsl:template name="MakeTblStyle">
    ...
    <xsl:text>.</xsl:text><xsl:value-of select="$styleId"/><xsl:value-of select="$tblStyleSuffix"/>
    <xsl:text>{</xsl:text><xsl:call-template name="RecursiveApplyTblPr.class"/><xsl:text>} </xsl:text>

    <xsl:text>.</xsl:text><xsl:value-of select="$styleId"/><xsl:value-of select="$rowStyleSuffix"/>
    <xsl:text>{</xsl:text><xsl:call-template name="RecursiveApplyTrPr.class"/><xsl:text>} </xsl:text>

    <xsl:text>.</xsl:text><xsl:value-of select="$styleId"/><xsl:value-of select="$cellStyleSuffix"/>
...
<xsl:template match="/w:document">
			...
			<html>
				<head>
					<xsl:for-each select="$ndOfficeDocPr/o:HyperlinkBase[1]"><base href="{(.)}"/></xsl:for-each>
          ...
          <xsl:if test="w:bgPict/w:background/@w:bgcolor">
					...
					<xsl:if test="//w:body//w:footnote">
						...
						<xsl:for-each select="//w:body//w:footnote">
							<a> 
								<xsl:attribute name="href"><xsl:text>#</xsl:text><xsl:value-of select="$footnoteRefLink" /><xsl:value-of select="position() + $start" /></xsl:attribute>
								<xsl:attribute name="target"><xsl:text>_self</xsl:text></xsl:attribute>				
								<xsl:attribute name="name"><xsl:value-of select="$footnoteLink" /><xsl:value-of select="position() + $start" /></xsl:attribute>
								<xsl:text>[</xsl:text><xsl:value-of select="position() + $start" /><xsl:text>]</xsl:text>
							</a>
							<xsl:apply-templates select="*" />
						</xsl:for-each>
					</xsl:if>
					...
					<xsl:if test="//w:body//w:endnote">
						...
						<xsl:for-each select="//w:body//w:endnote">
							<a> 
								<xsl:attribute name="href"><xsl:text>#</xsl:text><xsl:value-of select="$endnoteRefLink" /><xsl:value-of select="position() + $start" /></xsl:attribute>
								<xsl:attribute name="target"><xsl:text>_self</xsl:text></xsl:attribute>				
								<xsl:attribute name="name"><xsl:value-of select="$endnoteLink" /><xsl:value-of select="position() + $start" /></xsl:attribute>
								<xsl:text>[</xsl:text><xsl:value-of select="position() + $start" /><xsl:text>]</xsl:text>
							</a>
							<xsl:apply-templates select="*" />
						</xsl:for-each>
	...
	<xsl:template match="w:drawing">
		...
		<xsl:variable name="maxImgWidth" select="500"/>
		<xsl:variable name="maxImgHeight" select="450"/>
		<xsl:choose>
			<xsl:when test="round(number($w) div 9525) &gt; $maxImgWidth or round(number($h) div 9525) &gt; $maxImgHeight">
				<a href="?image={.//a:blip/@r:embed[1]}" rel="lightbox" title="">
					<img src="?image={.//a:blip/@r:embed[1]}&amp;w={$maxImgWidth}&amp;h={$maxImgHeight}" alt=""  border="0" />
				</a>
			</xsl:when>
			<xsl:otherwise>
				<img src="?image={.//a:blip/@r:embed[1]}&amp;w={round(number($w) div 9525)}&amp;h={round(number($h) div 9525)}" alt="" border="0" />
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
